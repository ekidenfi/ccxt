...
class ekiden extends Exchange {
    ...
    public function compute2_4h_volumes_from_candles(array $market): PromiseInterface {
        return Async\async(function () use ($market) {
            try {
                $since = $this->milliseconds() - 24 * 60 * 60 * 1000;
                $candles = Async\await($this->fetch_ohlcv($market['symbol'], '1m', $since, 1440));
                $baseSum = 0;
                $quoteSum = 0;
                $infoMarket = $market['info'] || array();
    ...
    public function fetch_positions(?array $symbols = null, $params = array ()): PromiseInterface {
        return Async\async(function () use ($symbols, $params) {
            ...
            $response = Async\await($this->v1PrivateGetUserPortfolio ($params));
    ...
    public function fetch_ticker(string $symbol, $params = array ()): PromiseInterface {
        return Async\async(function () use ($symbol, $params) {
            ...
            if ($missingQuote && $missingBase) {
                $fromCandles = Async\await($this->compute2_4h_volumes_from_candles($market));
    ...
    public function create_order(string $symbol, string $type, string $side, float $amount, ?float $price = null, $params = array ()): PromiseInterface {
        return Async\async(function () use ($symbol, $type, $side, $amount, $price, $params) {
            ...
            if ($hasPayload) {
                ...
                if ($commitProvided) {
                    $responseProvided = Async\await($this->v1PrivatePostUserIntentCommit ($request));
                } else {
                    $responseProvided = Async\await($this->v1PrivatePostUserIntent ($request));
                }
            ...
            if ($reduceOnlyParam) {
                try {
                    $allPositions = Async\await($this->fetch_positions(null, array()));
            ...
            if ($postOnlyParam) {
                ...
                try {
                    $ob = Async\await($this->fetch_order_book($market['symbol'], 1));
            ...
            $responseSigned = null;
            if ($commitFlag) {
                $responseSigned = Async\await($this->v1PrivatePostUserIntentCommit ($request));
            } else {
                $responseSigned = Async\await($this->v1PrivatePostUserIntent ($request));
            }
    ...
    public function cancel_order(string $id, ?string $symbol = null, $params = array ()): PromiseInterface {
        return Async\async(function () use ($id, $symbol, $params) {
            ...
            if ($commitFlag) {
                $responseSigned2 = Async\await($this->v1PrivatePostUserIntentCommit ($request));
            } else {
                $responseSigned2 = Async\await($this->v1PrivatePostUserIntent ($request));
            }
