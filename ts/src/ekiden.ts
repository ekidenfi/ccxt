...
import { ArgumentsRequired, OrderNotFound, NotSupported, InvalidOrder } from './base/errors.js';
import type { Market, OHLCV, Order, Int, Ticker, Num, OrderSide, OrderType, Dict, Str, Trade, OrderBook, Balances, Tickers, Strings, Position, Bool } from './base/types.js';
...
export default class ekiden extends Exchange {
    ...
    private fromDecimals (valueInt: number, decimals: number): number {
    ...
    private applyPriceMultiplier (price: number): number {
    ...
    private parseTickerStats (response: Dict, market: Market): Ticker {
    ...
    private async compute24hVolumesFromCandles (market: Market): Promise<Dict> {
        try {
    ...
    async fetchOrder (id: string, symbol: Str = undefined, params = {}): Promise<Order> {
        ...
        const market = this.market (this.normalizeSymbol (symbol));
        const request: Dict = { 'market_addr': market['id'], 'per_page': 100 };
        const response = await this.v1PrivateGetUserOrders (this.extend (request, params));
        for (let i = 0; i < response.length; i++) {
    ...
    async createOrder (symbol: string, type: OrderType, side: OrderSide, amount: number, price: Num = undefined, params = {}): Promise<Order> {
        ...
        const postOnlyParam: Bool = this.safeBool (params, 'postOnly');
        const reduceOnlyParam: Bool = this.safeBool (params, 'reduceOnly');
        ...
        if (reduceOnlyParam) {
            try {
                ...
                let posForMarket: Position = undefined;
    ...
    async setLeverage (leverage: number, symbol: Str = undefined, params = {}) {
        ...
        if (!this.secret) {
            throw new NotSupported (this.id + ' setLeverage() requires either params { payload, signature, nonce } or exchange.secret to sign the intent');
        }
        const commitFlag = this.safeBool (params, 'commit', true);
        const payload: Dict = { 'type': 'leverage_assign', 'leverage': leverage, 'market_addr': market['id'] };
        const nonce = ('nonce' in params) ? this.safeInteger (params, 'nonce') : this.milliseconds ();
        request = this.buildSignedIntent (payload, nonce);
        let responseSigned = undefined;
        if (commitFlag) {
